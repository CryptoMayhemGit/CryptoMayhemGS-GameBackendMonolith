...
{
    internal class ExploreMissionRepositoryTests : BaseRepositoryTests
    {
        ...
        public void Setup()
        {
            exploreMissionRepository = GetExploreMissionRepository();
        }
        ...
        public async Task RemoveMission_WhenMissionsExist_ThenGetThem_Test()
        {
            using (IDbConnection db = new SqlConnection(ConnectionString))
            {
                ...
                string addMission = $"insert into mission.ExploreMission (NpcId, LandId, UserId, FinishDate) values ({npcId},{landId},{userId},GETUTCDATE()); SELECT CAST(SCOPE_IDENTITY() as int)";
                int missionId = await db.QuerySingleAsync<int>(addMission);

                await exploreMissionRepository.RemoveMissionAsync(missionId);

                string getMission = "select * from mission.ExploreMission";
